 ...
 *   - Yes / Dismiss buttons
 *   - A shrinking progress bar showing remaining time
 *   - A fade-out in the last ~400 ms
 *
 * The event detail may include: { content, platform, postId }
 */
export default function SavedPostToast() {
  const [toast, setToast] = useState(null); // null or { content, platform, postId }
  ...
  useEffect(() => {
    const handler = (e) => {
      // If a toast is already showing, dismiss it first then show new one
      clearTimeout(timerRef.current);
      cancelAnimationFrame(rafRef.current);
      setFading(false);
      setProgress(100);
  ...
  const handleSendToChat = () => {
    if (toast?.content) {
      sessionStorage.setItem(
        "chibitek-pending-chat-post",
        JSON.stringify({ content: toast.content, platform: toast.platform })
      );
    }
    dismiss();
    navigate("/chat");
  ...
  return (
    ...
    >
      {/* Progress bar */}
      <div style={{ height: 3, background: "#e9ecef", position: "relative" }}>
        <div
          style={{
            height: "100%",
            width: `${progress}%`,
